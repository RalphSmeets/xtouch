...
def get_fw_version(source, target, env):
    global XTOUCH_VER_MAJ
    global XTOUCH_VER_MIN
    global XTOUCH_VER_REV

    for item in buildFlags.get("CPPDEFINES"): 
        if (type(item) is list):
            if (item[0]=="XTOUCH_VER_MAJ"): XTOUCH_VER_MAJ = item[1]
            if (item[0]=="XTOUCH_VER_MIN"): XTOUCH_VER_MIN = item[1]
            if (item[0]=="XTOUCH_VER_REV"): XTOUCH_VER_REV = item[1]
...
def copy_merge_bins(source, target, env):
    version = 'v' + str(XTOUCH_VER_MAJ) + '.' + str(XTOUCH_VER_MIN) + '.' + str(XTOUCH_VER_REV) + '_' + get_firmware_commit_hash()
